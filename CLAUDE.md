...
- **Voice picker (ships in release):** the pronunciation voice is a single `RecordedVoice` (`FlashcardSettings.recordedVoice`) selectable in Settings — `current` = Google Neural2-C, `kore` = Google Chirp3-HD Kore, `matilda` = ElevenLabs Matilda (eleven_v3, **default**), and `device` = the live Apple system voice (`AVSpeechSynthesizer`). There is no separate recorded/device *source* toggle anymore — device is just another voice (the old `AudioSource` enum and `fc_audioSource` key were retired; `FlashcardSettings.migrateLegacyAudioSourceIfNeeded` folds a legacy `device` source into `recordedVoice = .device`). The device voice is **only offered when a Thai system voice is installed** (`AudioPlayer.isThaiVoiceAvailable`) and, if selected but later unavailable, playback falls back to Neural2. It speaks `AudioPlayer.liveText(...)` (a different text derivation than the curated clips) — note that for tone marks/rules its tone accuracy is unreliable, so the recorded voices remain the pedagogical baseline. Recorded voices bundle MP3s. The app bundle flattens all resources to the root (the synchronized file group does not preserve subfolders — `subdirectory:` never actually matched, hence the root fallback), so voices are disambiguated by a **filename suffix**, not folders: Neural2 keeps the canonical `cheat_sheet_<type>_<key>.mp3` names (what `sound_inventory.py`/`generate_sounds.py`/the docs catalog expect); alternates append `_kore`/`_matilda`. A missing alternate clip falls back to the current (Neural2) set. Matilda's set is generated by `scripts/generate_elevenlabs_sounds.py`. That script strips the space from consonant names by default (`คอ ควาย` → `คอควาย`): eleven_v3 renders the space as an unnaturally long mid-word pause (Google voices handle it as a natural beat, so their consonants keep the space). eleven_v3 also appends variable leading/trailing silence, so the bundled Matilda consonants were silence-trimmed via `silencedetect`. Attribution now credits Google Cloud TTS AND ElevenLabs (About screen + APP_STORE_METADATA); ElevenLabs' commercial license applies to audio generated while subscribed (paid plan).
