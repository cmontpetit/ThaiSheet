...
### Persisted progress IDs (durable, content-independent)
- SRS progress is keyed by `FlashcardItem.id`, which is **structural**, never display text. Consonants/clusters key on their glyph; tone marks on `mark:class`; tone rules on `rule.id#sample.id`; vowels on `vowelID:duration:form`.
- **Vowel rows (`vowels.json`), tone-rule samples (`tone-rules.json`), and clusters (`clusters.json`) carry an explicit `id`.** Cluster ids are letters plus position (`กร:initial`, `ทร:final`) — free of the display dash notation. These are opaque, never displayed, and part of users' stored progress keys: **never rename one, never reuse a retired one**, and give a new row or sample a new id. Because identity lives in the data, reordering rows and inserting new ones is free — no positional indices are involved.
- Correcting content (a transcription, note, sample word, or written form) does NOT orphan progress.
- `ThaiSheet/Services/LegacyProgressIDs.swift` is a **frozen** snapshot of the keys shipped releases wrote (v1.0-build4 through v1.2.0-build8 all wrote the same keys — same derivation sources, same data). Never regenerate it from current content; that would defeat its purpose. When a future release changes an ID scheme, freeze that release's map as a second table and merge it in.
- Per-item **voice overrides** (`fc_voiceOverrides`) key on a row's identity, not a card's, so they have their own frozen table (`LegacyProgressIDs.voiceOverridesUpToV1_2`, vowels + clusters). `FlashcardSettings.migrateVoiceOverrideIDs` runs on every load and reload — including one triggered by an iCloud change — and writes the normalized form back.
- `ProgressIDMigration.migrate(_:using:)` applies the frozen map on every decode — locally in `LearningModel.load` (which then persists the normalized keys) and for cloud blobs in `SyncedKeyValueStore` reconciliation, so both sides are normalized before merging. It is idempotent, lossless (unknown keys are kept, never dropped), and collision-safe (most recently reviewed wins). It constructs no catalog to run.
- Duplicate durable IDs across the catalog are a data bug that would collide progress — `ProgressIDMigrationTests` fails if any appear.
...
- Naming: `cheat_sheet_{type}_{key}_{voice}.mp3` (e.g., `cheat_sheet_consonant_ก_neural2.mp3`, `cheat_sheet_vowel_กัน_matilda.mp3`)
...
- **Voice picker (ships in release):** the pronunciation voice is a single `RecordedVoice` (`FlashcardSettings.recordedVoice`) selectable in Settings — `neural2` = Google Neural2-C, `kore` = Google Chirp3-HD Kore, `matilda` = ElevenLabs Matilda (eleven_v3, **default**), and `device` = the live Apple system voice (`AVSpeechSynthesizer`). There is no separate recorded/device *source* toggle anymore — device is just another voice (the old `AudioSource` enum and `fc_audioSource` key were retired; `FlashcardSettings.migrateLegacyAudioSourceIfNeeded` folds a legacy `device` source into `recordedVoice = .device`). The legacy persisted `current` identifier migrates to `neural2`, including per-item overrides. The device voice is **only offered when a Thai system voice is installed** (`AudioPlayer.isThaiVoiceAvailable`) and, if selected but later unavailable, playback falls back to Matilda. It speaks `AudioPlayer.liveText(...)` (a different text derivation than the curated clips) — note that for tone marks/rules its tone accuracy is unreliable, so the recorded voices remain the pedagogical baseline. Recorded voices bundle MP3s. The app bundle flattens all resources to the root (the synchronized file group does not preserve subfolders — `subdirectory:` never actually matched, hence the root fallback), so every set uses an explicit filename suffix: `_neural2`, `_kore`, or `_matilda`. Missing recorded clips follow the explicit playback order selected voice → Matilda → Neural2, with duplicates removed; inventory tests require all three complete sets. Matilda's set is generated by `scripts/generate_elevenlabs_sounds.py`. That script strips the space from consonant names by default (`คอ ควาย` → `คอควาย`): eleven_v3 renders the space as an unnaturally long mid-word pause (Google voices handle it as a natural beat, so their consonants keep the space). eleven_v3 also appends variable leading/trailing silence, so the bundled Matilda consonants were silence-trimmed via `silencedetect`. Attribution now credits Google Cloud TTS AND ElevenLabs (About screen + APP_STORE_METADATA); ElevenLabs' commercial license applies to audio generated while subscribed (paid plan).
...
- Do not bundle audio recorded from Apple System Voices. Apple speech is synthesized live on-device only; bundled recordings come from the documented Google Cloud TTS and ElevenLabs generators.
...
- Reference sample words are deduplicated by Thai word and generated as `cheat_sheet_sample_word_{word}_neural2.mp3` in the production directory; `--sample-words` generates only that set and `--all` includes it. Voice-neutral candidate directories keep the canonical unsuffixed inventory names.
...
# Dry run (no credentials) to preview every provider's planned columns
python3 scripts/generate_tts_comparison.py \
  --provider google --provider azure --provider elevenlabs --provider openai --dry-run

# Multi-provider comparison — each provider's credentials are read only from
# the environment; providers without keys can simply be omitted.
AZURE_SPEECH_KEY=... AZURE_SPEECH_REGION=... \
ELEVENLABS_API_KEY=... OPENAI_API_KEY=... \
  python3 scripts/generate_tts_comparison.py \
    --provider google --provider azure --provider elevenlabs --provider openai --force
```

Supported providers and their credentials/voices:
- `google` (default) — uses the existing `gcloud` application-default auth. Default
  columns are the bundled Neural2-C voice, `th-TH-Standard-A`, and several Chirp 3 HD
  speakers (Aoede, Kore, Charon, Puck, Leda). Override with `--google-voice`.
- `azure` — `AZURE_SPEECH_KEY` + `AZURE_SPEECH_REGION`; defaults to Premwadee, Achara,
  Niwat. Override with `--azure-voice`.
- `elevenlabs` — `ELEVENLABS_API_KEY`; model `eleven_v3`. IMPORTANT: Thai is ONLY
  supported by `eleven_v3` — `eleven_multilingual_v2` and the flash/turbo v2.5 models
  do NOT support Thai and produce garbled output (verified via `/v1/models`). The
  public voice library has no Thai-native voices, so defaults are clear premade voices
  (Alice/Matilda/Daniel) on v3. Voice IDs are opaque, so pass
  `--elevenlabs-voice 'Label=voice_id'` for readable columns. Discovering/listing
  library voices needs a key with the "Voices" (read) permission; a TTS-only key can
  still synthesize with a known voice ID.
- `openai` — `OPENAI_API_KEY`; model `gpt-4o-mini-tts`, defaults to alloy/shimmer.
  Override with `--openai-voice`. Thai is unofficial — audit isolated syllables/tones.

An unavailable or invalid voice name only skips that column (it renders as
"Not generated"); it no longer aborts the whole run. The manifest's `Tones` section
includes a controlled 5-tone minimal set on ก plus real-word tone contrasts so tone
differentiation can be judged directly. Keep provider output local until it has been
reviewed by a native Thai speaker and its distribution terms have been confirmed
(ElevenLabs, OpenAI, and Azure all permit commercial use of generated audio under
their terms, but confirm at bundling time).

For a complete candidate set, generate a local bundled-versus-candidate review
