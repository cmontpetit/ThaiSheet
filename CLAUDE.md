...
- The Audio setting selects one source for all sounds: Recorded voice uses bundled MP3s (including sample words); Device voice uses live `AVSpeechSynthesizer` with a Thai system voice. If no Thai system voice is available, playback falls back to the recorded set. (The recorded/Device source toggle is DEBUG-only.)
- **Recorded-voice picker (ships in release):** three bundled voice sets are selectable in Settings via `RecordedVoice` (`FlashcardSettings.recordedVoice`): `current` = Google Neural2-C, `kore` = Google Chirp3-HD Kore, `matilda` = ElevenLabs Matilda (eleven_v3) — **Matilda is the default**. The app bundle flattens all resources to the root (the synchronized file group does not preserve subfolders — `subdirectory:` never actually matched, hence the root fallback), so voices are disambiguated by a **filename suffix**, not folders: Neural2 keeps the canonical `cheat_sheet_<type>_<key>.mp3` names (what `sound_inventory.py`/`generate_sounds.py`/the docs catalog expect); alternates append `_kore`/`_matilda`. A missing alternate clip falls back to the current (Neural2) set. Matilda's set is generated by `scripts/generate_elevenlabs_sounds.py`. That script strips the space from consonant names by default (`คอ ควาย` → `คอควาย`): eleven_v3 renders the space as an unnaturally long mid-word pause (Google voices handle it as a natural beat, so their consonants keep the space). eleven_v3 also appends variable leading/trailing silence, so the bundled Matilda consonants were silence-trimmed via `silencedetect`. Attribution now credits Google Cloud TTS AND ElevenLabs (About screen + APP_STORE_METADATA); ElevenLabs' commercial license applies to audio generated while subscribed (paid plan).
