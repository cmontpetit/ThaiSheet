...
- Entries with the exact same synthesis input reuse one processed MP3 within a full generation run, preventing generative voices from producing different takes for equivalent app entries. `--check-files` rejects inconsistent duplicates; repair an existing set with `--unify-identical-inputs` (add `--output-dir scratchpad/...` for a candidate set). Repair prefers sample-word takes, then open-vowel takes, then complete tone/cluster entries.
